...
def get_data(baseurl, resource="", params={}):
    ...
    file = requests.get(baseurl + resource, params)
    a_dict = file.json()
    return a_dict
...
def search_swapi(resource, query):
    ...
    return get_data('https://swapi.co/api/', resource, params={'search': query})



def get_information_on_characters(character_list):
    """
    get_information_on_characters:
        parameters:
            - list_of_characters (list)
    ...
    for item in character_list:
        nested_dict[item['name']] = {'birth_year':item['birth_year'],
                                     'species_name': get_data(item['species'][0])['name'],
                                     'homeworld_name': get_data(item['homeworld'])['name']
                                     }
    return nested_dict
...
def main():
    ...
    result_dict = search_swapi('people', 'darth')
    dict_darth= get_information_on_characters(result_dict['results'])
    write_json('darth_info.json', dict_darth)

    result_dict = search_swapi('people', 'skywalker')
    dict_sky = get_information_on_characters(result_dict['results'])
    write_json('skywalker_info.json', dict_sky)

    dict_tat= search_swapi('planets', 'tatooine')
    tat_list = dict_tat['results'][0]['residents']
    empty_list= []
    for url in tat_list:
        variable= get_data(url)
        empty_list.append(variable)

    dict_tat= get_information_on_characters(empty_list)
    write_json('tatooine_residents_info.json', dict_tat)
...
print(search_swapi('people','yoda'))
